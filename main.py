...
class DatabaseManager:
    ...
    def mark_device_as_unknown(self, mac):
        ...
        try:
            self.neo4j_manager.device_manager.mark_device_as_unknown(mac)
...
class NetworkAnalyzerApp(ctk.CTk):
    ...
    def unblock_device_action(self, ip):
        ...
        if self.wifi_blocker:
            # Get MAC before unblocking to sync with DB
            mac = None
            if ip in self.wifi_blocker.blocked_devices:
                 mac = self.wifi_blocker.blocked_devices[ip]['mac']
                 
            self.wifi_blocker.unblock_device(ip)
            self.refresh_blocked_list()
            self.log(f"Unblocked device: {ip}")
            
            if mac and self.db_manager.use_neo4j:
                 try:
                     self.db_manager.neo4j_manager.device_manager.set_device_blocked_status(mac, False)
                 except: pass
